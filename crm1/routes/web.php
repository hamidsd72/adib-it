...
Route::get('test_sms', function () {
 \App\Sms::SendSms('tst','09387383528');
});

Route::get('test', function () {
    return $recentWorks=Work::where('user_id',auth()->id())->orderByDesc('updated_at')->take(5)->get();

    return \App\Project::find(465);
});
Route::get('contract', function () {
    $contracts = \App\Contract::where('active', 1)->orderBy('id')->get();
...
Route::group(['prefix' => 'panel', 'middleware' => ['auth', 'roles', 'suspended'], 'roles' => ['Management', 'Administrative__Department', 'Technical__Department', 'Financial__Department', 'Support__Department', 'Sale__Department']], function () {
    ...
    Route::get('job_notify', function () {
        $user = Auth::user();
        $stringBuilder = "";
        $jobs = \App\Job::where('status', 1)->where('referred_to', $user->id)->get();
    ...
    Route::get('delete_user/{id}', 'Panel\UserController@delete')->name('delete_user');
    ...
    Route::get('packages', 'Panel\PhaseController@packages');
    ...
    Route::get('comment-confirm/{id}/{type}', 'Panel\TicketController@comment_confirm')->name('comment-confirm');
